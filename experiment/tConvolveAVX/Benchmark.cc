                           ...
                           const int gSize)
{
    for (int dind = 0; dind < int(data.size()); ++dind) {
        ...
        simd::complex4<float> cvis_vec = {.d = {dre, dim, dre, dim, dre, dim, dre, dim}};
        ...
        for (int suppv = 0; suppv < sSize[wind]; suppv++) {
            ...
            simd::complex4<float> *conv_vec =
                reinterpret_cast<simd::complex4<float> *>(&C[cind]);
            simd::complex4<float> *grid_vec = reinterpret_cast<simd::complex4<float> *>(
                &grid[gind]);

            int rem = sSize[wind] % 4;
            int tiles = sSize[wind] / 4;
            ...
            //     uoff = suppu + support;
            //     Value wt = convFunc(uoff, voff);
            //     grid(iu + suppu, iv + suppv) += cVis * wt;
            // }

            for (int suppu = tiles * 4; suppu < sSize[wind]; suppu++) {

                           ...
                           const int gSize)
{
    for (int dind = 0; dind < int(data.size()); ++dind) {
        ...
        simd::complex2<float> cvis_vec = {.d = {dre, dim, dre, dim}};
        ...
        for (int suppv = 0; suppv < sSize[wind]; suppv++) {
            ...
            simd::complex2<float> *conv_vec =
                reinterpret_cast<simd::complex2<float> *>(&C[cind]);
            simd::complex2<float> *grid_vec = reinterpret_cast<simd::complex2<float> *>(
                &grid[gind]);

            int rem = sSize[wind] % 2;
            int tiles = sSize[wind] / 2;
            ...
            for (int suppu = tiles * 2; suppu < sSize[wind]; suppu++) {

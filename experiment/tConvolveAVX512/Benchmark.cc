...
///
/// @author M Nimalan   <m.nimalan@thoughtworks.com>
...
void Benchmark::gridKernel(std::vector<Value>& C,
                           ...
                           const int gSize)
{
    for (int dind = 0; dind < int(data.size()); ++dind) {
        ...
        simd::complex8<float> cvis_vec = {
            .d = {dre, dim, dre, dim, dre, dim, dre, dim,
                dre, dim, dre, dim, dre, dim, dre, dim}};
        ...
        for (int suppv = 0; suppv < sSize[wind]; suppv++) {
            ...
            simd::complex8<float> *conv_vec =
                reinterpret_cast<simd::complex8<float> *>(&C[cind]);
            simd::complex8<float> *grid_vec = reinterpret_cast<simd::complex8<float> *>(
                &grid[gind]);

            int rem = sSize[wind] % 8;
            int tiles = sSize[wind] / 8;
            ...
            for (int suppu = tiles * 8; suppu < sSize[wind]; suppu++) {
